...
func TestSendDiscordNotificationIfNeeded(t *testing.T) {
	...
	err := MockDiscordHealthTracker.SendDiscordNotificationIfNeeded(shared.HEALTHY, generations, time.Now(), time.Now(), false, false)
	assert.Nil(t, err)
	...
	err = MockDiscordHealthTracker.SendDiscordNotificationIfNeeded(shared.UNHEALTHY, generations, time.Now(), time.Now(), false, false)
	assert.Nil(t, err)
