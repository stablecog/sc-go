...
func main() {
	...
	if *loadQdrant {
		...
		loaded := 0
		for loaded < 50000 {
			rawQ := fmt.Sprintf("select id, image_path, upscaled_image_path, gallery_status, embedding, is_favorited, generation_id, created_at, updated_at from generation_outputs where embedding is not null and has_embeddings = false order by created_at desc limit 100 offset %d;", loaded)
