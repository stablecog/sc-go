...
func NewTranslatorSafetyChecker(ctx context.Context, redis *database.RedisWrapper, openaiKey string, disable bool) *TranslatorSafetyChecker {
	return &TranslatorSafetyChecker{
		Ctx:              ctx,
		TargetFloresUrl:  utils.GetEnv().PrivateLinguaAPIUrl,
		TranslatorCogUrl: utils.GetEnv().TranslatorCogURL,
...
// TranslatePromptsBulk - Translates a bulk of prompts with caching and error handling.
type PromptGen struct {
	ID   uuid.UUID
	Text string
}

func (t *TranslatorSafetyChecker) TranslatePromptsBulk(prompts []PromptGen) ([]PromptGen, error) {
	translatedPrompts := make([]PromptGen, len(prompts))

	for i, prompt := range prompts {
		// Generate cache key
		cacheKey := "translate:" + prompt.Text

		// Attempt to fetch the translation from cache
		cachedTranslation, cacheErr := t.GetFromCache(cacheKey)
		if cacheErr == nil && cachedTranslation != "" {
			translatedPrompts[i] = PromptGen{
				ID:   prompt.ID,
		...
		// If not in cache, translate the prompt
		translatedPrompt, _, translateErr := t.TranslatePrompt(prompt.Text, "")
		if translateErr != nil {
			// Log error but use original prompt as the translation
			log.Errorf("Error translating prompt '%s': %v", prompt, translateErr)
			translatedPrompts[i] = PromptGen{
				ID:   prompt.ID,
				Text: prompt.Text,
			} // Use original prompt if translation fails
		} else {
			// Cache the translation if successful and not identical to the original
			if translatedPrompt != prompt.Text {
				cacheErr := t.CacheTranslation(cacheKey, translatedPrompt)
				if cacheErr != nil {
					log.Errorf("Error caching translation for '%s': %v", prompt, cacheErr)
				}
			}
			translatedPrompts[i] = PromptGen{
				ID:   prompt.ID,
	...
	return translatedPrompts, nil
}

// GetFromCache attempts to fetch a translated prompt from Redis cache.
func (t *TranslatorSafetyChecker) GetFromCache(key string) (string, error) {
	// Implementation for fetching from Redis
	return t.Redis.Client.Get(t.Redis.Ctx, key).Result()
}

// CacheTranslation caches a translated prompt in Redis.
func (t *TranslatorSafetyChecker) CacheTranslation(key string, value string) error {
	// Implementation for caching in Redis
	return t.Redis.Client.Set(t.Redis.Ctx, key, value, time.Hour*48).Err()
}
