...
// QuerySubscriptionTier chains the current query on the "subscription_tier" edge.
func (sq *SubscriptionQuery) QuerySubscriptionTier() *SubscriptionTierQuery {
	query := (&SubscriptionTierClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) Clone() *SubscriptionQuery {
	...
	return &SubscriptionQuery{
		...
		withSubscriptionTier: sq.withSubscriptionTier.Clone(),
...
// WithSubscriptionTier tells the query-builder to eager-load the nodes that are connected to
// the "subscription_tier" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SubscriptionQuery) WithSubscriptionTier(opts ...func(*SubscriptionTierQuery)) *SubscriptionQuery {
	query := (&SubscriptionTierClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubscriptionTier = query
	return sq
}
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := sq.withSubscriptionTier; query != nil {
		if err := sq.loadSubscriptionTier(ctx, query, nodes, nil,
			func(n *Subscription, e *SubscriptionTier) { n.Edges.SubscriptionTier = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadSubscriptionTier(ctx context.Context, query *SubscriptionTierQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *SubscriptionTier)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
		fk := nodes[i].SubscriptionTierID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(subscriptiontier.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subscription_tier_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
